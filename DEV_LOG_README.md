...
- 왜 개선했는지: 성장 리포트가 사용자/수업 맥락이 섞이거나, 재처리 시 같은 노트의 임베딩이 중복 저장될 수 있었습니다.
- 어떻게 개선했는지: pgvector 지원 DB 이미지를 사용하고, 임베딩 검색 범위에 `user_id`, `category_id`, `folder_id`를 적용했으며, 재저장 전 기존 note embedding을 삭제하고 노트 삭제 시 embedding도 함께 정리했습니다.
- 어떤 효과가 있는지: 성장 리포트가 더 명확한 비교 범위 안에서 생성되고, 재처리/삭제 후에도 벡터 데이터가 불필요하게 쌓이지 않습니다.

### 성장 리포트 비교 품질 개선

- 왜 개선했는지: 유사한 과거 피드백을 가져와도 단순히 반복 지적으로만 해석하면 사용자가 실제로 무엇이 달라졌는지 알기 어려웠습니다.
- 어떻게 개선했는지: 성장 리포트 프롬프트에 과거 피드백과 현재 피드백의 문제 위치, 강도, 과제 구체성, 반복 여부를 먼저 비교하도록 지시했습니다.
- 어떤 효과가 있는지: 성장 리포트가 단순한 반복 지적보다 “이전과 비교해 무엇이 달라졌는지”를 더 구체적으로 설명할 수 있습니다.

### 성장 리포트 RAG 컨텍스트 개선

- 왜 개선했는지: 레슨이 적을 때는 최근 수업 흐름 자체가 성장 리포트 품질에 더 큰 영향을 줄 수 있었습니다.
- 어떻게 개선했는지: 새 메타데이터를 추가하지 않고 기존 임베딩 row의 `created_at`을 활용해 최근 과거 레슨 2개를 함께 전달하고, 유사 검색 결과는 유사도와 최근성을 섞어 정렬했습니다.
- 어떤 효과가 있는지: 성장 리포트가 장기적으로 비슷한 피드백을 찾는 동시에, 사용자가 방금 이어서 받은 수업 흐름도 더 자연스럽게 반영할 수 있습니다.
